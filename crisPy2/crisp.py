...
class CRISPSpectroPol(CRISPSpectro):
    '''
    This is the class for a singular CRISP cube described by one WCS which has full spectropolarimetry information.
    ...
    def __str__(self):
        if self.meta is None:
            ...
            cl = str(np.round(self.meta.get("TWAVE1")))
            wwidth = self.meta.get("WWIDTH1")
            shape = str([self.meta.get("NAXIS4"), self.meta.get("NAXIS3"), self.meta.get("NAXIS2"), self.meta.get("NAXIS1")])
            el = self.meta.get("WDESC1")
            pointing_x = str(self.meta.get("CRVAL1"))
            pointing_y = str(self.meta.get("CRVAL2"))

            return f"""CRISP Imaging Spectropolarimetry
            -------------------------
            {date} {time}
